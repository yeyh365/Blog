<template>
  <div class="hbl-fa">
    ...
    <div v-for="(item, index) in commentList" :key="index" class="hbl-child">
          ...
          <avatar
            :avatar="
              item.FromUserInfo.Photo
                ? $utils.imgUrl(item.FromUserInfo.Photo)
                : avatar
            "
            @click="toUserInfo(item.FromUserInfo.Id)"
          ></avatar>
        </div>

        <div class="comment-f">
          <div>
            <div class="nickname author" @click="toUserInfo(item.FromUserInfo.Id)">
              {{ item.FromUserInfo.Name }}
            </div>
            <div v-if="item.FromUserInfo.Id === authorId" class="icon author">
              {{ label }}
            </div>
            <div class="date">
              {{ item.Created }}
      ...
      <div class="children" v-for="(ritem, index) in item.Children" :key="index">
        <div class="reply"></div>
        <div class="content">
          <div class="comment-f">
            <avatar
              :avatar="
                ritem.FromUserInfo.Photo
                  ? $utils.imgUrl(ritem.FromUserInfo.Photo)
                  : avatar
              "
            ></avatar>
          </div>
          ...
          <div class="comment-f">
            <div>
              <div class="nickname author">
                {{ ritem.FromUserInfo.Name }}
              </div>
              <div v-if="ritem.FromUserInfo.Id === authorId" class="icon author">
                {{ label }}
              </div>
              <div class="date">
                {{ ritem.Created }}
